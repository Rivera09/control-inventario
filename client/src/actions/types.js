export const SET_ALERT = 'SET_ALERT';
export const REMOVE_ALERT = 'REMOVE_ALERT';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAIL = 'LOGIN_FAIL';
export const USER_LOADED = 'USER_LOADED';
export const AUTH_ERROR = 'AUTH_ERROR';
export const PROVIDER_CREATED = 'PROVIDER_CREATED';
export const PROVIDER_CREATION_FAILED = 'PROVIDER_CREATION_FAILED';
export const USER_CREATED = 'USER_CREATED';
export const USER_CREATION_FAILED = 'USER_CREATION_FAILED';
export const LOGGED_OUT = 'LOGGED_OUT';
export const CLIENT_CREATED = 'CLIENT_CREATED';
export const CLIENT_CREATION_FAILED = 'CLIENT_CREATION_FAILED';
export const PRODUCT_CREATED = 'PRODUCT_CREATED';
export const PRODUCT_CREATION_FAILED = 'PRODUCT_CREATION_FAILED';